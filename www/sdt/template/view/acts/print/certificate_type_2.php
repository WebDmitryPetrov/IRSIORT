...
//        $pdf->Image($_SERVER['DOCUMENT_ROOT'] . "/fpdf/certifikat_cut.jpg",0,0,210,158);
    ...
    $pdf->SetX($x);
//    $pdf->Cell(0, 37, '', $borders, 2, 'R'); //отступ
    $pdf->Cell(0, 39, '', $borders, 2, 'R'); //отступ
    $pdf->Cell($wide_cell+$wide_cell_enhancer+2, 3, '', $borders, 0, 'С'); //отступ слева
    $pdf->Cell($wide_cell-$wide_cell_enhancer-13, 3, $person['fio_rus'], $borders, 2, $person['fio_align']); //ФИО рус




    $pdf->SetX($x);
//    $pdf->Cell(0, 4.5, '', $borders, 2, 'R'); //отступ
    $pdf->Cell(0, 2.5, '', $borders, 2, 'R'); //отступ
    $pdf->Cell($wide_cell+$wide_cell_enhancer+2, 3, '', $borders, 0, 'C'); //отступ слева
    $pdf->Cell($wide_cell-$wide_cell_enhancer-13, 3, $person['fio_lat'], $borders, 2, $person['fio_align']); //ФИО лат
...
//    $pdf->Cell($wide_cell-$wide_cell_enhancer, 3, $person['uroven'], $borders, 2, 'C'); //уровень тестирования
    $pdf->Cell($wide_cell-$wide_cell_enhancer, 3, '', $borders, 2, 'C'); //уровень тестирования



    $pdf->SetX($x);
//    $pdf->Cell(0, 19.5, '', $borders, 2, 'R'); //отступ
    $pdf->Cell(0, 41, '', $borders, 2, 'R'); //отступ
    $pdf->Cell($wide_cell+$wide_cell_enhancer+2, 3, '', $borders, 0, 'C'); //отступ слева
    $pdf->Cell($wide_cell-$wide_cell_enhancer-13, 3, $person['org'], $borders, 2, 'C'); //Учреждение
	
	 /*Уровень*/
    $pdf->Cell($wide_cell+$wide_cell_enhancer+2, 3, '', $borders, 2, 'C'); //отступ слева
    $pdf->Cell($wide_cell-$wide_cell_enhancer-13, 3, $person['uroven'], $borders, 2, 'C'); //уровень тестирования
/*конец уровень*/
...
//    $pdf->Cell(0, 13, '', $borders, 2, 'R'); //отступ
    $pdf->Cell(0, 6, '', $borders, 2, 'R'); //отступ


    $pdf->Cell(15, 3, '', $borders, 0, 'C'); //Дата печати
    $pdf->Cell(35, 3, $person['city'], $borders, 0, 'C'); //Дата печати
    $pdf->Cell(21, 3, '', $borders, 0, 'C'); //отступ
    $pdf->Cell(23, 3, $person['document_nomer'], $borders, 1, 'C'); //срок сертификата
    $pdf->Cell(0, 5, '', $borders, 2, 'R'); //отступ
    $pdf->SetX($x);
    $pdf->Cell(15, 3, '', $borders, 0, 'C'); //Дата печати


    $pdf->Cell(35, 8.5, $person['print_date'], $borders, 0, 'C'); //Дата печати
//    $pdf->Cell(36, 6, '', $borders, 0, 'C'); //отступ
    $pdf->Cell(21, 8.5, '', $borders, 0, 'C'); //отступ
    $pdf->Cell(23, 8.5, $person['cert_date'], $borders, 0, 'R'); //срок сертификата
    $pdf->Cell(4.5, 8.5, '', $borders, 0, 'C'); //отступ
    $pdf->Cell(51, 8.5, $person['rukovod_dolzhn_1'].' '.$person['rukovod_dolzhn_2'], $borders, 0, 'C'); //должность
    $pdf->Cell(47.5, 0.5, $person['rukovod_fio'], $borders, 0, 'R'); //руководитель
    $pdf->Cell(13, 3, '', $borders, 2, 'R'); //отступ
