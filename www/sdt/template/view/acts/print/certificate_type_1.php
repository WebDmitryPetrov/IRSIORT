...
foreach ($people as $person)
    {
        ...
        $blank_type=new BlankType();
        $template_blank_type=$blank_type->getBlankType($cert_type,$person['blank_number']);
        if (empty($template_blank_type->id)) {
            //die($cert_type);
            if (empty($blank_type->getDefaultBlankType($cert_type))) $template_blank_type = 1;
            else $template_blank_type = $blank_type->getDefaultBlankType($cert_type);

        }
        else $template_blank_type = $template_blank_type->id;

        //getDefaultBlankType($cert_type);
require ('certificate_type_1_template_'.$template_blank_type.'.php');
/*
