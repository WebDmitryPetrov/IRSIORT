...
if (empty($Act->getUniversity()->getParentedFieldValue('print_invoice_quoute'))) {
    $platelschik = $Act->getUniversity()->getLegalInfo()['form'] . ' ' . $Act->getUniversity()->getLegalInfo()['name_parent'];
} else {
    $platelschik = $Act->getUniversity()->getLegalInfo()['form'] . ' &laquo;' . $Act->getUniversity()->getLegalInfo()['name_parent'] . '&raquo;';
}
//Адрес плательщика
//$platelschik_adr = $Act->getUniversity()->legal_address;
$platelschik_adr = $Act->getUniversity()->getLegalInfo()['legal_address_parent'];
//Счет номер
...
//Дата договора
if (!empty($Act->getUniversityDogovor()->date) && $Act->getUniversityDogovor()->date != '0000-00-00') {
    $dogovor_date = 'от ' . date('d.m.Y', strtotime($Act->getUniversityDogovor()->date)) . ' г.';
} else {
    $dogovor_date = '';
}
...
if (\SDT\logic\Prices\Nds::isNeedNDS($Act))
{
    ...
    $nds_text_1 = '';
    $nds_text_2='<tr>
            <td valign="middle" colspan="4" style="border-top: 1px solid black; text-align: right; height: 40px">
                <div>В том числе НДС '.$nds->getNdsValue().'%</div>
            </td>

            <td valign="middle" style="border-top: 1px solid black; text-align: right;" >
                <div><span style="font-size: 14.0pt;">'.$nds->getNdsPart($schet_sum).'</span></div>
            </td>

        </tr>';
    $nds_text_3=', в т.ч.НДС '.$nds->getNdsValue().'%';

}
else
{
    $nds_text_1 = '<br><div align=center style="padding: 5pt 0 10pt 0">НДС НЕ ОБЛАГАЕТСЯ</div>';
    $nds_text_2 = '';
    $nds_text_3 = ' БЕЗ НДС';
}
...
//$nds = str_replace('.', ',', sprintf("%01.2f", ($sum_total - $sum_total / 1.20)));
...
$compareActInvoiceDate = new \DateTime($Act->invoice_date);
$compareMinDate = new \DateTime('19.04.2017');

if($Act->test_level_type_id==2 && ($compareActInvoiceDate >= $compareMinDate )){
    $za_chto = '<div align=center style="padding: 5pt 0 5pt 0">Дополнительные образовательные услуги<br>по проведению комплексного экзамена</div>';
}
...
<div style="width: 180mm; margin:0;padding:0">
            ...
            <td width="46%" valign="top"
                ...
                <div>
							<span style="font-size: 12.0pt"><?php echo $za_chto ?>
                                ...
                                <!--                                <div align=center style="padding: 5pt 0 10pt 0">НДС НЕ ОБЛАГАЕТСЯ</div>-->
                                <?=$nds_text_1;?>
							</span>
                </div>
            </td>
    ...
    */ ?>
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="601"
           style="wi dth:450.75pt;width:100%;margin-left:4.65pt;border-collapse:collapse;" class="table4">
        <tbody>
        ...
        <tr style="height:12.75pt">
            ...
            <td width="168" valign="top" colspan="8" rowspan="3"
                ...
                <div><span style="font-size:10.0pt"><?
                        if (empty(strpos(TEXT_HEADCENTER_BIK,' '))) echo TEXT_HEADCENTER_BIK;
                        else echo substr_replace(TEXT_HEADCENTER_BIK,'<br>',strpos(TEXT_HEADCENTER_BIK,' '),1);
                        ?></span></div>
            </td>
        </tr>
        <tr style="height:12.75pt">
        ...
        <tr style="height:12.75pt">
            <td width="601" valign="top" colspan="18"
                ...
                <div><span style="font-size:10.0pt">Оплата по сч. № <?= $podr_index; ?> / <?= $schet_num ?>
                        от <?= $schet_date ?> г. за дополнительные образовательные услуги по проведению тестирования согл. дог. № <?= $dogovor_num ?>
                        <!--от   -->
                        <?= $dogovor_date ?><!-- г.--><br>Акт от <?= date("d.m.Y", strtotime($Act->testing_date)) ?> г.<?=$nds_text_3;?></span>
                </div>
            </td>
        </tr>
        <tr>
