...
if ($echo_cert==1 && empty($Man->blank_number)) continue;
   ...
   <td>
   '.$Man->getSurname_rus().' '.$Man->getName_rus().' / '.$Man->getSurname_lat().' '.$Man->getName_lat().'
   </td>';

    $blank_type=@BlankType::getBlankType($Act->test_level_type_id,$Man->getBlank_number());
    ...
    if ($echo_cert==1) echo '<td>'.$echo_blank_type.$Man->getBlank_number().'</td>';
    echo '</tr>';
}
