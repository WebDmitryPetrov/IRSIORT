...
<tr class="js-act-row
	<?
//if ($item->is_printed): echo 'success'; endif;
...
?>
	">
    <!--    <td><a name="--><? //= $item->id ?><!--"></a><span class="js-act-number">-->
    <? //= $item->number ?><!--</span></td>-->
    ...
    */
    ...
    <td class="">
        ...
        <? if ($item->state == DublAct::STATE_IN_HC): ?>
            <a data-id="<?php echo $item->id; ?>"
               class="btn invoice btn-primary  btn-block" target="_blank"
               href="dubl.php?action=dubl_act_numbers&id=<?php echo $item->id; ?>">Присвоить номера дубликатам и
                печать</a>
            <div></div>
...
*/ ?>
            <a data-id="<?php echo $item->id; ?>"
               ...
               href="dubl.php?action=dubl_act_processed&id=<?php echo $item->id; ?>"
               onclick="return confirm('Вы уверены?')">Завершить сессию</a>
            <div></div>
