...
//Плательщик
//if (empty($University->print_invoice_quoute))
if (empty($University->getParentedFieldValue('print_invoice_quoute'))) {
    $platelschik = $University->getLegalInfo()['form'] . ' ' . $University->getLegalInfo()['name_parent'];
} else {
    $platelschik = $University->getLegalInfo()['form'] . ' &laquo;' . $University->getLegalInfo()['name_parent'] . '&raquo;';
}
//Адрес плательщика
//$platelschik_adr = $University->legal_address;
$platelschik_adr = $University->getLegalInfo()['legal_address_parent'];
//Счет номер
$schet_num = $Act->invoice;
$podr_index = $Act->invoice_index;
//var_dump($schet_num); die();
//Дата счета
$schet_date = date('d.m.Y', strtotime($Act->invoice_date));


/* $schet_date = new DateTime(date('d.m.Y', strtotime($Act->invoice_date)));
 $schet_date = $Act->getPrintDateAfterCheckDate($schet_date);
 $schet_date = date('d.m.Y', strtotime($schet_date));
*/

//Договор номер
$dogovor_num = $University_dogovor->number;
//Дата договора
if (!empty($University_dogovor->date) && $University_dogovor->date != '0000-00-00') {
    $dogovor_date = 'от ' . date('d.m.Y', strtotime($University_dogovor->date)) . ' г.';
} else {
    $dogovor_date = '';
}
// было так до 25.02.15 $dogovor_date = date('d.m.Y', strtotime($Act->getUniversityDogovor()->date));
...
{
    if (ActMan::getByID($man->old_man_id)->is_anull()) continue;
    $kolichestvo++;
}
...
//Сумма счета
$schet_sum = str_replace('.', ',', sprintf("%01.2f", $per));
$dubl_price_00 = str_replace('.', ',', sprintf("%01.2f", $dubl_price));
//Наименование-дата
...
$signing = ActSigning::getByID($Act->signing);

//var_dump($Act);


$nds_val=new \SDT\logic\Prices\Nds($Act->accepted_date);
//$nds=$nds_val->getNdsPart($dubl_price);
//$nds_total = $nds_val->getNdsPart($sum_total);
$nds = str_replace('.', ',', $nds_val->getNdsPart($dubl_price));
$nds_total = str_replace('.', ',', $nds_val->getNdsPart($sum_total));
...
//$za_chto = '<div align=center style="padding: 5pt 0 5pt 0">Дополнительные образовательные услуги<br>по проведению тестирования</div>';
$za_chto = '<div align=center style="padding: 5pt 0 5pt 0">Услуга за оформление дубликата сертификата</div>';
...
<div style="width: 180mm; margin:0;padding:0">
    <table cellspacing="0" cellpadding="0" border="1"
        ...
        <tr class=toptable>
            <td width="100%" valign="top"
                ...
                <div>
							<span>Сч.№ <?= TEXT_HEADCENTER_CHECK_N ?>
</span>
                </div>
                <div>
							<span><?= TEXT_HEADCENTER_BANK_1 ?> БИК <?= TEXT_HEADCENTER_BIK ?>
</span>
                </div>
                <div>
        ...
        <tr>
            <td width="46%" valign="top"
                style="width: 46.32%; border: solid windowtext 1.0pt; border-top: none; bor der-bottom: 0">
                <div>
            ...
            <td width="12%" style="vertical-align:middle; border-bottom: 0">
                <div align="center">
							<span style="font-size: 14.0pt;"> <?= $dubl_price_00 ?> <br>
						</span>
                </div>
            </td>

            <td width="20%" style="vertical-align:middle; border-bottom: 0">
                <div align="center">
        ...
        <tr>
            <td>
                <div align=center style="padding: 5pt 0 10pt 0">в т. ч. НДС <?=$nds_val->getNdsValue();?>%</div>
            </td>

            <td>
                <div align="center">
							<span style="font-size: 14.0pt; margin-right: 2.85pt;"><?= $nds ?>
							</span>
                </div>
            </td>
            ...
            <td>
                <div align="center">
							<span style="font-size: 14.0pt; margin-right: 2.85pt;"><?= $nds_total ?>
							</span>
                </div>
            </td>
        </tr>
        ...
        <td align=right style="border-top: 0"><span style="font-size: 14.0pt;"><div style="padding: 5pt 0 5pt 0"><?= str_replace('.', '-', $nds); ?></div></span></td>
    */ ?></tr>
        <tr>
            <td width="100%" valign="top" colspan="5">
                <div>
    ...
    <div style="margin-bottom: 6.0pt; width: 100%; font-weight: bold;padding-left: 10px">
        <span style="font-size: 16.0pt; float: left;"><?php echo $signing->position ?></span><span
                style="font-size: 16.0pt; float: right; margin-right: 20px"><?php echo $signing->caption ?></span>
    </div>
    <br>
    ...
    <table border="0" cellspacing="0" cellpadding="0" width="601"
        ...
        <tr style="height:12.75pt;">
            <td width="287" valign="bottom" nowrap="" colspan="3"
                style="width: 150pt; padding: 0cm 0pt; height: 12.75pt; background-position: initial initial; background-repeat: initial initial;">
            ...
            <td width="44" valign="bottom" nowrap="" colspan="2"
                style="padding: 0cm 5.4pt; height: 12.75pt; width: 0pt;">
                <div>&nbsp;</div>
            </td>
        ...
        <tr style="height:12.75pt">
            <td width="601" valign="top" colspan="18"
                ...
                <div><span style="font-size:10.0pt">Оплата по сч. № <?= $podr_index; ?> / <?= $schet_num ?>
                        от <?= $schet_date ?>
                        г. за услугу по оформлению дубликата сертификата. дог. № <?= $dogovor_num ?> <!--от   -->
                        <?= $dogovor_date ?><!-- г.-->
                      п.п. 6.8.   НДС <?=$nds_val->getNdsValue();?> %</span>
                </div>
            </td>
        </tr>
        <tr>
            <td width="703" valign="bottom" colspan="18"
                style="width: 527pt; border-style: none none solid; border-bottom-color: windowtext; border-bottom-width: 1pt; padding: 0cm 5.4pt; he ight: 12.75pt; background-position: initial initial; background-repeat: initial initial;">
                <div><span style="font-size:10.0pt">Назначение платежа</span></div>
            </td>
        </tr>
        <tr>
            <td valign="bottom" colspan="18" rowspan="3"
                style=" border: none; pad ding: 0 5.4pt;  background-position: initial initial; background-repeat: initial initial;">
