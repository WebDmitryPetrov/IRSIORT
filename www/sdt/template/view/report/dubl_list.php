<h1>Реестр актов тестовых сессий по дубликатам</h1>
...
$i=0;
    ...
    <tr class="<?if ($item->isDeleted()):?>error<?endif?>">
        ...
        <td><?php echo $C->date($item->created);?></td>
    ...
    <!--    <td><?php /*echo $item->paid ? 'Да' : 'Нет'; */?></td>
        ...
        <td><?php echo $C->dateTime($item->getCheckDate(),true) ?></td>
        <td><?php echo $item->deleted?'Удален':''?></td>
        <? if(Roles::getInstance()->userHasRole(Roles::ROLE_CONTR_BUH)):?>
            <td><a target="_blank" href="index.php?action=buh_dubl_view_act&id=<?=$item->id?>" class="btn btn-primary btn-mini">Просмотреть</a> </td>
        <?endif?>
    </tr>
