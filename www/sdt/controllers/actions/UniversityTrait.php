...
trait UniversityTrait
{
    ...
    protected function university_add_action()
    {
        ...
        if (count($_POST)) {
            ...
            $validation = new \SDT\Validation\University();
            $errors = $validation->validateParent($univer);
            if (!$errors) {
                $univer->save();
                ...
                $_SESSION['flash'] = 'Заполните все обязательные поля';
                $univer->setValidateErrors($errors);
            }
        }
    ...
    {
        if (empty(filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT)) || !is_numeric(
...
//            die('sdf');
            if (!$errors) {
                ...
                $_SESSION['flash'] = 'Локальный центр сохранён';
                $this->redirectByAction('university_view', $params);
            } else {
                $_SESSION['flash'] = 'Заполните все обязательные поля';
                $univer->setValidateErrors($errors);
    ...
    protected function university_child_view_action()
    {
        ...
        return $this->render->view(
            'universities/children/view',
            array('object' => $univer, 'haveChildren' => $hasChildren)
        );
    }
...
//die(var_dump($univer->getAvailableUsers()));
    ...
    protected function university_child_edit_action()
    {
        ...
        if (count($_POST)) {
            $univer->parseParameters($_POST);
            $validation = new \SDT\Validation\University();
            $errors = $validation->validateChild($univer);
            if (!$errors) {
                $univer->save();
                $params = array('id' => $univer->id);
                if ($univer->user_password) {
