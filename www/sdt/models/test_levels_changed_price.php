...
class ChangedPriceTestLevel extends Model
{
    ...
    static function getByID($id)
    {
        ...
        $sql = 'select id,caption,type_id, test_group, is_publicated from sdt_test_levels  where deleted=0';
        //die ($sql);
        $result = mysql_query($sql);
        while ($res = mysql_fetch_array($result)) {
            ...
            if (!empty($list[$res['id']]['sub_test_price_2'])) {
                $sub_test_price_2 = $list[$res['id']]['sub_test_price_2'];
            } else {
                $sub_test_price_2 = '';
            }
...
//die (var_dump($univer));
    ...
    public function save($to_save)
    {
        ...
        foreach ($to_save as $save) {
            ...
            if ($save['sub_test_price_2'] != 0) {
                $sub_test_price_2 = floatval(
                    str_replace(',', '.', $save['sub_test_price_2'])
                );
            } else {
...
//новое
        ...
        {
            $university_id='t1.university_id';
        }
...
t3.sub_test_price_2 as changed_sub_test_price_2,
t2.is_price_change,
t4.price as normal_price,
t4.sub_test_price as normal_sub_test_price,
t4.sub_test_price_2 as normal_sub_test_price_2

from sdt_act as t1
left join sdt_university as t2 on ' . $university_id . '=t2.id
left join test_levels_changed_price as t3 on ' . $university_id . '=t3.univer_id and t3.test_level_id=' . $level_id . '
...
//        die(var_dump($sql));
        ...
        $array['sub_test_price_2'] = $row['normal_sub_test_price_2'];
        ...
        if ($row['is_price_change'] == 1) {
            ...
            if (!empty($row['changed_sub_test_price_2']) && $row['changed_sub_test_price_2'] != 0) {
                $array['sub_test_price_2'] = $row['changed_sub_test_price_2'];
            }
        }
//        (var_dump($array));
