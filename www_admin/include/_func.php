...
/*function db_connect()
{
    ...
    $con = @mysql_connect($db_serv, $db_user, $db_password)or die(debag_trac(
            "Проверте кофигурацию: Ардрес базы данных, логин и пароль.",
            mysql_error()
        ));

    @mysql_select_db($db_name, $con) or die(debag_trac(
...
{
    if (!mysql_query("SET CHARACTER SET cp1251")) {
        echo mysql_error();
    }
    if (!mysql_query("SET character_set_connection = cp1251")) {
        echo mysql_error();
    }
    if (!mysql_query("set character_set_results = cp1251 ")) {
        echo mysql_error();
    }
    if (!mysql_query("set character_set_server =  cp1251")) {
        echo mysql_error();
    }
...
function db_encoding_utf8()
{
    ...
    if (!mysql_query("SET character_set_connection = utf8")) {
        echo mysql_error();
    }
    if (!mysql_query("set character_set_results = utf8 ")) {
        echo mysql_error();
    }
    if (!mysql_query("set character_set_server =  utf8")) {
        echo mysql_error();
    }
}*/
...
#		if(isset($_COOKIE['s_id']) or (!empty($s_id) and $s_id != '1'))
        ...
        if (!isset($_SESSION['u_id']) || empty($_SESSION['u_id'])) {
            ...
            $sql = "select u_id, concat(surname,' ',firstname,' ', fathername) as userName, surname, univer_id, head_id,user_type_id
            from tb_users where login='$login' and password=md5(md5('$password')) and (head_id=0 or head_id is Null)";
//die($sql);
            $res = mysql_query($sql) or die(debag_trac(
                "Проверка пользователя - неудачна, по причине что нету таблицы в БД tb_users",
                mysql_error()
            ));
            $id = 0;
            $userName = '';
...
/*выкидывает, если в основную админку полезет ФМС-пользователь /*diff1*/
        if (!empty($privelegies['fms_user']) && empty($privelegies['fms_admin']))
        {
            ses_destr();
            header('Location:' . $link . '?ooops_auth=3');
            die;
...
//die (var_dump($_SESSION));
        if ($_SESSION['u_id']==MAINEST_ADMIN_ID) {
            $privelegies['admin_head'] = 1;
        }

        $_SESSION['privelegies'] = $privelegies;
        //var_dump($privelegies);
...
function auth1($link = '../index.php')
{
    ...
    if (isset($_GET['exit']) && $_GET['exit'] == "exit") {
        ses_destr();
        //echo 'stop1';
        header('Location: ' . $link . '?ooops=x4');die;
...
#		if(isset($_COOKIE['s_id']) or (!empty($s_id) and $s_id != '1'))
        ...
        if (!isset($_SESSION['u_id']) || empty($_SESSION['u_id'])) {
            if (isset($_POST['login']) && !empty($_POST['login'])) {
                ...
                ses_destr();
                header('Location:' . $link . '?ooops_auth=1');die;
            ...
            $sql = "select u_id, concat(surname,' ',firstname,' ', fathername) as userName, surname, univer_id, head_id
            from tb_users where login='$login' and password=md5(md5('$password'))
            and
            (head_id = 0 or head_id is Null)";
//die($sql);
            $res = mysql_query($sql) or die(debag_trac(
                "Проверка пользователя - неудачна, по причине что нету таблицы в БД tb_users",
                mysql_error()
            ...
            if ($id !=0) {
                ...
                #setcookie('s_id',session_id(),time()+60*60);
                //setcookie('PHPSESSID',session_id(),time()+60*60,"/");
            } else {

                ses_destr();
                header('Location:' . $link . '?ooops_auth=3');die;
        ...
        $sql = "select g_id from tb_groups, tb_users, tb_relations where u_id=" . $_SESSION['u_id'] . " and u_id=fk_u_id and g_id=fk_g_id";
        $res = mysql_query($sql)or die(debag_trac(' Проверте настройку конфига! ', mysql_error()));
        $c1 = array_flip($groups);

        while ($row = mysql_fetch_array($res)) {
            if (in_array($row['g_id'], $groups)) {
                $privelegies[$c1[$row['g_id']]] = $row['g_id'];
            } else {
                $privelegies[$row['g_id']] = $row['g_id'];
            }
        }
//        die(var_dump($privelegies));
      /*  if(CURRENT_HEAD_CENTER!=$_SESSION['head_id'] and !array_key_exists('admin',$privelegies)){
            ses_destr();
            header('Location:' . $link . '?ooops_auth=3');
...
//var_dump($_SESSION['privelegies']);die;
        redir_prava();
    }
