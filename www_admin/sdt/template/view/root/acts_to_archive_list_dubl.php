...
Акты ПО ДУБЛИКАТАМ, созданные до(включительно):
...
foreach ($array as $hc)
{
    ...
    if ($count > 0)
    {
       $button='<a class="btn btn-danger btn-block" onclick="return confirm(\'Будут отправлены в архив до '.$till.' (включительно).Вы уверены?\');" href="index.php?action=set_archive_by_head_dubl&id='.$hc['id'].'&till='.$till.'" >В архив</a>';
    }
    else
    {
//       $button='';
