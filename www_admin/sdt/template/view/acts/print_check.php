...
{
$platelschik = $Act->getUniversity()->form . ' ' . $Act->getUniversity()->name;
}
else
{
...
$nds = str_replace('.', ',', sprintf("%01.2f", ($sum_total - $sum_total / 1.18)));
...
$pol_name_full =$head_center_text->long_ip;
$pol_name_short = $head_center_text->short_ip;
$pol_addr = $head_center_text->address_1;
...
<table cellspacing="0" cellpadding="0" border="1"
    ...
    <tr class=toptable>
        <td width="100%" valign="top"
            ...
            <div>
							<span>ОКВЭД <?=$head_center_text->okved?>,
                                ОКПО <?=$head_center_text->okpo?>,
                                ОКТМО <?=$head_center_text->oktmo?>,
                                ОГРН <?=$head_center_text->ogrn?>
                            </span>
            </div>
            <div>
							<span>Юридический адрес: <?=$head_center_text->legal_address_1?>
                            </span>
            </div>
            <div>
							<span>Получатель: <?=$head_center_text->check_receiver?>
</span>
            </div>
            <div>
							<span>Сч.№ <?=$head_center_text->check_n?>  БИК <?=$head_center_text->bik?>
</span>
            </div>
            <div>
							<span><?=$head_center_text->bank_1?>
</span>
            </div>
            <div>
							<span>ИНН <?=$head_center_text->bank_inn?>
                                КПП <?=$head_center_text->bank_kpp?>
</span>
            </div>
            <div style="mar gin-bottom:20px">
							<span>КБК <?=$head_center_text->bank_kbk?>
</span>
            </div>
...
<table border="0" cellspacing="0" cellpadding="0" width="601"
    ...
    <tr style="height:12.75pt">
        <td width="361" colspan="6"
            ...
            <div><span style="font-size:10.0pt"><?=$head_center_text->bank_2?></span></div>
        </td>
        ...
        <td width="168" valign="top" colspan="8" rowspan="3"
            ...
            <div><span style="font-size:10.0pt"><?=$head_center_text->bik?></span></div>
        </td>
    </tr>
    <tr style="height:12.75pt">
        <td width="361" colspan="6"
            ...
            <div><span style="font-size:10.0pt"><?=$head_center_text->bank_3?></span></div>
        </td>
    ...
    <tr style="height:12.75pt">
        <td width="287" valign="bottom" colspan="3"
            ...
            <div><span style="font-size:10.0pt">ИНН&nbsp; <?=$head_center_text->bank_inn?></span></div>
        </td>
        <td width="74" valign="bottom" colspan="3"
            ...
            <div><span style="font-size:10.0pt">КПП&nbsp; <?=$head_center_text->bank_kpp?></span></div>
        </td>
        <td width="72" valign="top" colspan="4" rowspan="2"
            style="width: 53.95pt; border-style: none solid solid none; border-bottom-color: black; border-bottom-width: 1pt; border-right-color: black; border-right-width: 1pt; padding: 0cm 5.4pt; height: 12.75pt; background-position: initial initial; background-repeat: initial initial;">
            <div><span style="font-size:10.0pt">Сч. №</span></div>
        ...
        <td width="168" valign="top" colspan="8" rowspan="2"
            ...
            <div><span style="font-size:10.0pt"><?=$head_center_text->check_n?></span></div>
        </td>
    </tr>
    <tr style="height:12.75pt">
        <td width="361" valign="top" colspan="6" rowspan="2"
            style="width: 270.8pt; border-style: none solid; border-left-color: windowtext; border-left-width: 1pt; border-right-color: black; border-right-width: 1pt; padding: 0cm 5.4pt; height: 12.75pt; background-position: initial initial; background-repeat: initial initial;">
            <div><span style="font-size:10.0pt"><?=$head_center_text->check_receiver?></span></div>
        </td>
    </tr>
    <tr style="height:12.75pt">
    ...
    <tr style="height:12.75pt">
        <td width="169"
            ...
            <div><span style="font-size:10.0pt"><?=$head_center_text->bank_kbk?></span></div>
        </td>
        <td width="96" nowrap=""
            ...
            <div><span style="font-size:10.0pt"><?=$head_center_text->oktmo?></span></div>
        </td>
    ...
    <tr style="height:12.75pt">
        <td width="601" valign="top" colspan="18"
            ...
            <div><span style="font-size:10.0pt">Оплата по сч. № <?= $podr_index; ?> / <?= $schet_num ?>
                    от <?= $schet_date ?> г. за дополнительные образовательные услуги по проведению тестирования согл. дог. № <?= $dogovor_num ?> <!--от   --><?= $dogovor_date ?><!-- г.--><br>Акт от <?= date("d.m.Y", strtotime($Act->testing_date)) ?> г. БЕЗ НДС<?/*в т.ч. НДС 18% -   <?= str_replace('.', '-', $nds); ?> руб.*/?></span>
            </div>
        </td>
    </tr>
    <tr >
