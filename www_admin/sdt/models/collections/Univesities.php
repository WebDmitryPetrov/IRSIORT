...
class Univesities extends ArrayObject
{
    ...
    public static function getByHORG($hc)
    {
        $list = new Univesities();
        $sql = 'SELECT su.* FROM sdt_university su
 LEFT JOIN sdt_head_center shc ON shc.id = su.head_id
 WHERE su.deleted=0 AND horg_id =  ' . intval($hc) . '
         ORDER BY name';
//        die($sql);
        $ci = Connection::getInstance();

        $result = $ci->query($sql);

        foreach ($result as $row) {
            $list[] = new University($row, true);
        }

        return $list;
    }

    public static function getByHead($hc, $withDeleted = false)
    {
        $deletedSql = ' and su.deleted=0 ';
        if ($withDeleted)
            $deletedSql = '';
 ...
 WHERE   su.head_id =  ' . intval($hc) . $deletedSql . ' 
         ORDER BY name';
//        die($sql);
        $ci = Connection::getInstance();
...
WHERE
  sdt_act.state = \'' . mysql_real_escape_string($level) . '\'
    ...
    AND
         su.head_id = ' . self::get_CURRENT_HEAD_CENTER() . '
...
//        $list=[];
    ...
    public static function getByDubl($level, $test_level_type_id = null, $skipDeleted = true)
    {
        ...
        AND da.deleted=0
        ' . $restrict . '
        AND sdt_university.head_id=' . self::get_CURRENT_HEAD_CENTER() . '
...
//            $new->unread = $row['unread'];
    ...
    public static function getByLevels($level, $pfur_api = 0)
    {
        ...
        $pfur_api_sql = '';
        if (HeadCenter::getByID(self::get_CURRENT_HEAD_CENTER())->pfur_api) {
            $pfur_api_sql = ' and sdt_university.pfur_api =' . $pfur_api;
        }
...
WHERE
  ...
  sdt_act.state IN (\'' . ACT::STATE_RECEIVED . '\',\'' . ACT::STATE_PRINT . '\',\'' . ACT::STATE_WAIT_PAYMENT . '\',\'' . ACT::STATE_CHECK . '\' )
    AND
    sdt_university.head_id = ' . self::get_CURRENT_HEAD_CENTER() . '
...
, cc';
    ...
    static public function get4Act()
    {
        ...
        $sql = 'SELECT * FROM sdt_university
				WHERE deleted=0
				   AND
         head_id = ' . self::get_CURRENT_HEAD_CENTER() . '
				AND id IN (
...
//        die($sql);
    ...
    {
//        $list = new Univesities();
        $sql = 'SELECT su.id, su.name, count(c.id) AS have_child , su.pfur_api, su.api_enabled FROM sdt_university su 
...
//        die($sql);
        ...
        if (count($haveChildren)) {
            ...
            $sql = 'SELECT su.id, su.name, su.parent_id, su.pfur_api, su.api_enabled FROM sdt_university su WHERE su.deleted=0
     AND
         su.head_id = ' . self::get_CURRENT_HEAD_CENTER() . '
...
//            var_dump($childList);die;
    ...
    static public function search(array $condtitions = [])
    {

        $C = Connection::getInstance();
        $where = [];
        if (!empty($condtitions['inn'])) {
            $where[] = sprintf("su.inn = '%s'", $C->escape($condtitions['inn']));
        }

        if (!empty($condtitions['name'])) {
            $where[] = sprintf("(su.name like  '%%%s%%' or su.short_name like '%%%s%%')",
                $C->escape($condtitions['name']), $C->escape($condtitions['name']));
        }
        if(empty($where)) return [];
        $sql = 'SELECT su.id, su.name, count(c.id) AS have_child , su.pfur_api, su.api_enabled, shc.name  as hc_name FROM sdt_university su 
...
//        die($sql);
        ...
        $oldList = $C->query(sprintf($sql, implode(' and ', $where)));
        $list = [];
        if (empty($oldList)) return [];
        foreach ($oldList as $item) {
            $list[$item['id']] = $item;
...
SELECT
 ...
 ( tu.univer_id = 0 OR tu.univer_id IS NULL)
    ...
    AND
         tu.head_id = ' . self::get_CURRENT_HEAD_CENTER() . '
...
//        die($sql);
    ...
    {
        $head_center_result = mysql_result(mysql_query('SELECT head_id FROM tb_users WHERE u_id=' . $id), 0);
//echo $head_center_result; die;
        $sql = '
SELECT
...
//die($sql);
    ...
    private function get_CURRENT_HEAD_CENTER()
    {
        return (int)filter_input(INPUT_GET, 'h_id', FILTER_VALIDATE_INT);
    }
